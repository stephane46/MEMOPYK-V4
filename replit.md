...
- **Cropped Image Badge Fix COMPLETED (August 1, 2025)**: Fixed false positive cropped badges (✂️ Recadré EN/FR) appearing immediately after uploading new images. Badges now only display when a static image URL exists AND is different from the regular image URL, preventing misleading indicators when no actual cropping has been performed.
