...
- **MOBILE GALLERY CONSTRAINT SYSTEM FINALIZED (August 21, 2025)**: Corrected landscape video constraint logic - ALL videos now use width constraint (90% of screen width) on mobile devices instead of mixed height/width constraints. This prevents landscape videos like Safari from becoming too wide for mobile screens. Replaced Unicode play button triangle (▶) with proper SVG vectors to eliminate grey square background issues. Both desktop and mobile galleries now use consistent SVG play buttons with explicit fill colors.
