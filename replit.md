...
- **Video Analytics Duplicate Tracking Fix (v1.0.190)**: Resolved critical duplicate video analytics issue where single 16-second video watch created 2 views with 8-second average instead of 1 view with 16-second average. Root cause was multiple tracking calls: GallerySection.tsx (desktop & mobile click events) and VideoPlayer.tsx (play events) created 0-duration entries, while VideoOverlay.tsx created actual watch time entries. Removed all duplicate tracking calls - analytics now only track from VideoOverlay when users actually watch videos, not when they click to open lightbox. System now provides accurate single-entry tracking with correct watch duration and completion metrics.
