...
- **Single Cropping Interface IMPLEMENTED (August 1, 2025)**: Fixed UI logic inconsistency where "same video and photo" toggle still showed separate FR/EN cropping buttons. Now displays single purple-themed shared cropping interface when shared mode is enabled, with consolidated "Recadrer Photo Partagée" button that applies crop to both languages simultaneously. Enhanced admin interface consistency and user experience efficiency.
