...
## Recent Changes  
- **August 6, 2025**: **RESOLVED CRITICAL CACHE BUG** - Fixed cropping system cache invalidation issue where changes would revert after admin updates. Root cause was JSON cache file overriding database changes. Implemented automatic cache clearing after database updates in hybrid storage system. Cropping now works reliably: save to database → clear JSON cache → force fresh reads. All crop positions properly persist and sync across environments.
