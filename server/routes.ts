...
export async function registerRoutes(app: Express): Promise<void> {
  ...
  // ========================================
  // ADMIN SYNC ENDPOINTS - Database → JSON Export
  // ========================================

  // Admin: Sync Gallery Database to JSON
  app.post("/api/admin/gallery/sync-to-json", async (req, res) => {
    try {
      console.log('🔄 ADMIN SYNC (Gallery): Starting database → JSON export...');
      ...
      if (galleryItems.error) throw new Error(`Supabase query failed: ${galleryItems.error.message}`);
      if (!galleryItems.data || galleryItems.data.length === 0) {
        return res.status(404).json({ success: false, error: "No gallery items found in database" });
      }
      ...
      console.log(`✅ ADMIN SYNC (Gallery): Exported ${galleryItems.data.length} items to gallery.json`);
      
      galleryCache = null;
      res.json({ success: true, itemsExported: galleryItems.data.length, message: `Successfully synced ${galleryItems.data.length} gallery items to JSON` });
    } catch (error: any) {
      console.error('❌ ADMIN SYNC (Gallery) ERROR:', error);
      res.status(500).json({ success: false, error: error.message || "Failed to sync database to JSON" });
    }
  });

  // Admin: Sync Hero Videos Database to JSON
  app.post("/api/admin/hero-videos/sync-to-json", async (req, res) => {
    try {
      console.log('🔄 ADMIN SYNC (Hero Videos): Starting database → JSON export...');
      
      const heroVideos = await hybridStorage.supabase
        .from('hero_videos')
        .select('*')
        .order('id', { ascending: true });

      if (heroVideos.error) throw new Error(`Supabase query failed: ${heroVideos.error.message}`);
      if (!heroVideos.data || heroVideos.data.length === 0) {
        return res.status(404).json({ success: false, error: "No hero videos found in database" });
      }

      const jsonPath = path.join(process.cwd(), 'server/data/hero-videos.json');
      fs.writeFileSync(jsonPath, JSON.stringify(heroVideos.data, null, 2), 'utf8');
      console.log(`✅ ADMIN SYNC (Hero Videos): Exported ${heroVideos.data.length} items to hero-videos.json`);
      
      res.json({ success: true, itemsExported: heroVideos.data.length, message: `Successfully synced ${heroVideos.data.length} hero videos to JSON` });
    } catch (error: any) {
      console.error('❌ ADMIN SYNC (Hero Videos) ERROR:', error);
      res.status(500).json({ success: false, error: error.message || "Failed to sync database to JSON" });
    }
  });

  // Admin: Sync FAQs Database to JSON
  app.post("/api/admin/faqs/sync-to-json", async (req, res) => {
    try {
      console.log('🔄 ADMIN SYNC (FAQs): Starting database → JSON export...');
      
      const faqs = await hybridStorage.supabase
        .from('faqs')
        .select('*')
        .order('order_index', { ascending: true });

      const faqSections = await hybridStorage.supabase
        .from('faq_sections')
        .select('*')
        .order('order_index', { ascending: true });

      if (faqs.error) throw new Error(`Supabase FAQs query failed: ${faqs.error.message}`);
      if (faqSections.error) throw new Error(`Supabase FAQ Sections query failed: ${faqSections.error.message}`);

      const faqsPath = path.join(process.cwd(), 'server/data/faqs.json');
      const sectionsPath = path.join(process.cwd(), 'server/data/faq-sections.json');
      
      fs.writeFileSync(faqsPath, JSON.stringify(faqs.data || [], null, 2), 'utf8');
      fs.writeFileSync(sectionsPath, JSON.stringify(faqSections.data || [], null, 2), 'utf8');
      
      console.log(`✅ ADMIN SYNC (FAQs): Exported ${faqs.data?.length || 0} FAQs and ${faqSections.data?.length || 0} sections to JSON`);
      
      res.json({ 
        success: true, 
        itemsExported: (faqs.data?.length || 0) + (faqSections.data?.length || 0), 
        message: `Successfully synced ${faqs.data?.length || 0} FAQs and ${faqSections.data?.length || 0} sections to JSON` 
      });
    } catch (error: any) {
      console.error('❌ ADMIN SYNC (FAQs) ERROR:', error);
      res.status(500).json({ success: false, error: error.message || "Failed to sync database to JSON" });
    }
  });

  // Admin: Sync Legal Documents Database to JSON
  app.post("/api/admin/legal/sync-to-json", async (req, res) => {
    try {
      console.log('🔄 ADMIN SYNC (Legal Documents): Starting database → JSON export...');
      
      const legalDocs = await hybridStorage.supabase
        .from('legal_documents')
        .select('*')
        .order('type', { ascending: true });

      if (legalDocs.error) throw new Error(`Supabase query failed: ${legalDocs.error.message}`);

      const jsonPath = path.join(process.cwd(), 'server/data/legal-documents.json');
      fs.writeFileSync(jsonPath, JSON.stringify(legalDocs.data || [], null, 2), 'utf8');
      console.log(`✅ ADMIN SYNC (Legal Documents): Exported ${legalDocs.data?.length || 0} items to legal-documents.json`);
      
      res.json({ success: true, itemsExported: legalDocs.data?.length || 0, message: `Successfully synced ${legalDocs.data?.length || 0} legal documents to JSON` });
    } catch (error: any) {
      console.error('❌ ADMIN SYNC (Legal Documents) ERROR:', error);
      res.status(500).json({ success: false, error: error.message || "Failed to sync database to JSON" });
    }
  });

  // Admin: Sync CTA Settings Database to JSON
  app.post("/api/admin/cta/sync-to-json", async (req, res) => {
    try {
      console.log('🔄 ADMIN SYNC (CTA Settings): Starting database → JSON export...');
      
      const ctaSettings = await hybridStorage.supabase
        .from('cta_settings')
        .select('*')
        .order('id', { ascending: true });

      if (ctaSettings.error) throw new Error(`Supabase query failed: ${ctaSettings.error.message}`);

      const jsonPath = path.join(process.cwd(), 'server/data/cta-settings.json');
      fs.writeFileSync(jsonPath, JSON.stringify(ctaSettings.data || [], null, 2), 'utf8');
      console.log(`✅ ADMIN SYNC (CTA Settings): Exported ${ctaSettings.data?.length || 0} items to cta-settings.json`);
      
      res.json({ success: true, itemsExported: ctaSettings.data?.length || 0, message: `Successfully synced ${ctaSettings.data?.length || 0} CTA settings to JSON` });
    } catch (error: any) {
      console.error('❌ ADMIN SYNC (CTA Settings) ERROR:', error);
      res.status(500).json({ success: false, error: error.message || "Failed to sync database to JSON" });
    }
  });

  // Admin: Sync Why MEMOPYK Cards Database to JSON
  app.post("/api/admin/why-memopyk-cards/sync-to-json", async (req, res) => {
    try {
      console.log('🔄 ADMIN SYNC (Why MEMOPYK Cards): Starting database → JSON export...');
      
      const cards = await hybridStorage.supabase
        .from('why_memopyk_cards')
        .select('*')
        .order('orderIndex', { ascending: true });

      if (cards.error) throw new Error(`Supabase query failed: ${cards.error.message}`);

      const jsonPath = path.join(process.cwd(), 'server/data/why-memopyk-cards.json');
      fs.writeFileSync(jsonPath, JSON.stringify(cards.data || [], null, 2), 'utf8');
      console.log(`✅ ADMIN SYNC (Why MEMOPYK Cards): Exported ${cards.data?.length || 0} items to why-memopyk-cards.json`);
      
      res.json({ success: true, itemsExported: cards.data?.length || 0, message: `Successfully synced ${cards.data?.length || 0} Why MEMOPYK cards to JSON` });
    } catch (error: any) {
      console.error('❌ ADMIN SYNC (Why MEMOPYK Cards) ERROR:', error);
      res.status(500).json({ success: false, error: error.message || "Failed to sync database to JSON" });
    }
  });

  // Admin: Sync SEO Settings Database to JSON
  app.post("/api/admin/seo/sync-to-json", async (req, res) => {
    try {
      console.log('🔄 ADMIN SYNC (SEO Settings): Starting database → JSON export...');
      
      const seoSettings = await hybridStorage.supabase
        .from('seo_settings')
        .select('*');

      if (seoSettings.error) throw new Error(`Supabase query failed: ${seoSettings.error.message}`);

      const jsonPath = path.join(process.cwd(), 'server/data/seo-settings.json');
      fs.writeFileSync(jsonPath, JSON.stringify(seoSettings.data || [], null, 2), 'utf8');
      console.log(`✅ ADMIN SYNC (SEO Settings): Exported ${seoSettings.data?.length || 0} items to seo-settings.json`);
      
      res.json({ success: true, itemsExported: seoSettings.data?.length || 0, message: `Successfully synced ${seoSettings.data?.length || 0} SEO settings to JSON` });
    } catch (error: any) {
      console.error('❌ ADMIN SYNC (SEO Settings) ERROR:', error);
      res.status(500).json({ success: false, error: error.message || "Failed to sync database to JSON" });
    }
  });
