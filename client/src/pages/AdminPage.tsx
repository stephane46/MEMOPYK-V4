...
export default function AdminPage() {
  ...
  const sidebarItems = [
    ...
    { id: 'analytics', label: 'Analytics Legacy', icon: BarChart3 },
    { id: 'analytics-ga', label: 'Analytics GA4', icon: TrendingUp },
              ...
              <Card className="bg-gradient-to-r from-blue-50 to-orange-50 border border-orange-200">
                <CardContent className="p-6">
                  ...
                  <div className="flex flex-col items-center space-y-4">
                    <a
                      href="javascript:(function(){try{var k='ga_dev',b='ga-test-banner',on=localStorage.getItem(k)==='1';if(on){localStorage.removeItem(k);var e=document.getElementById(b);if(e)e.remove();if(typeof window.gtag==='function'){window.gtag('event','debug_toggle',{debug_mode:false,source:'bookmarklet'});}}else{localStorage.setItem(k,'1');var el=document.getElementById(b);if(!el){el=document.createElement('div');el.id=b;el.textContent='🧪 MEMOPYK TEST MODE';el.style.cssText='position:fixed;bottom:12px;right:12px;background:#ff9800;color:#fff;padding:8px 12px;font:600 13px/1.2 system-ui,Segoe UI,Roboto,Arial;border-radius:6px;box-shadow:0 6px 18px rgba(0,0,0,.2);z-index:2147483647;';document.body.appendChild(el);}else{el.style.display='block';}if(typeof window.gtag==='function'){window.gtag('event','debug_toggle',{debug_mode:true,source:'bookmarklet'});}}console.log('MEMOPYK Test bookmarklet executed successfully');}catch(err){console.error('Toggle failed:',err);}})();"
